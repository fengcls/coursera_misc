...
__global__ void matrixMultiplyShared(float *A, float *B, float *C, int numARows,
    ...
    // identify the row and column of C to work on
    ...
    for (int m = 0; m < (numAColumns-1)/TILE_WIDTH+1; ++m){
        if (Row < numARows && (m*TILE_WIDTH+threadIdx.x) < numAColumns){
            Ads[threadIdx.y][threadIdx.x]=A[Row*numAColumns + m*TILE_WIDTH + threadIdx.x];
        } else {
            Ads[threadIdx.y][threadIdx.x]=0.0;
        }
        if ((m*TILE_WIDTH + threadIdx.y) < numBRows && Col < numBColumns){
            Bds[threadIdx.y][threadIdx.x]=B[(m*TILE_WIDTH + threadIdx.y)*numBColumns+Col];
        } else {
...
int main(int argc, char **argv) {
    ...
    dim3 dimGrid(ceil(numCColumns),ceil(numCRows),1);
